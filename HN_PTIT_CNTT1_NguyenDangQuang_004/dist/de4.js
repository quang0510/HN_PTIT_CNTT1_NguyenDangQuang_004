...
class Pet {
    constructor(id, type, price, isAvailable) {
        this.id = id;
        this.type = type;
        this.price = price;
...
class Dog extends Pet {
    getSpecialCareInstructions() {
        const dog = [" chó thuần chủng , trại nhân giống uy tín"];
        return dog;
    }
    getOriginInfo() {
...
class Cat extends Pet {
    getSpecialCareInstructions() {
        const cat = ["Dọn khcaty cát thường xuyên , tiêm phòng định kỳ"];
        return cat;
    }
    getOriginInfo() {
...
class Bird extends Pet {
    getSpecialCareInstructions() {
        const bird = ["Cho ăn hạt và trái cây, Giữ lồng sạch sẽ"];
        return bird;
    }
    getOriginInfo() {
...
class Sale {
    constructor(saleID, customer, pet, quantity, totalPrice) {
        this.saleID = saleID;
...
class PetShop {
    constructor(pets, customers, sales) {
        this.pets = pets;
    ...
    addCustomer(name, email, phone) {
        const newCustomer = new Customer(this.customers[this.customers.length - 1].id++, name, email, phone);
